...
export const PayslipDocument = forwardRef<HTMLDivElement, PayslipDocumentProps>(
  ({ payslip, employeeSignatureSlot, managerSignatureSlot, rightSignatureLabel = "HR & Payroll Manager", className }, ref) => {
    ...
    return (
      <div ref={ref} className={cn("mx-auto w-full max-w-[920px] rounded-[28px] bg-white p-8 text-slate-900 shadow-2xl print:rounded-none print:shadow-none", className)}>
        <div className="overflow-hidden rounded-[24px] border border-slate-300 bg-white">
          <div className="border-b border-slate-300 px-8 py-8">
            <div className="flex items-start justify-between gap-6">
              <div>
                ...
                <p className="mt-2 text-lg font-medium text-slate-600">Tunisian ERP Payroll Module</p>
              </div>
              <div className="text-right">
                <p className="text-xs font-bold uppercase tracking-[0.35em] text-slate-500">Payslip</p>
                <h2 className="mt-2 text-3xl font-extrabold text-slate-900">{payslip.payrollPeriod?.periodName || "Payroll Period"}</h2>
                <p className="mt-2 text-sm text-slate-500">Generated on {formatDate(payslip.paymentDate || payslip.updatedAt)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="border-b border-slate-300 px-8 py-5 text-center">
            <h3 className="text-3xl font-black tracking-tight text-slate-900">
              Payslip for the month of {payslip.payrollPeriod?.periodName || "-"}
            </h3>
          </div>

          <div className="grid grid-cols-[1.3fr_1fr] border-b border-slate-300">
            <div className="border-r border-slate-300 px-8 py-6">
              <p className="mb-5 text-lg font-black uppercase tracking-wide text-slate-900">Employee Pay Summary</p>
              <div className="grid grid-cols-[220px_1fr] gap-y-3 text-[28px] leading-none text-slate-800 scale-[0.5] origin-top-left sm:scale-100 sm:text-lg">
                ...
                <span className="font-semibold">CIN / CNSS</span>
            ...
            <div className="px-8 py-6">
              <div className="flex h-full flex-col items-center justify-center text-center">
                <p className="text-2xl font-semibold text-slate-700">Employee Net Pay</p>
                <p className="mt-4 text-6xl font-black tracking-tight text-slate-900">{formatAmount(payslip.netSalary)}</p>
                <p className="mt-4 text-lg text-slate-600">
                  Gross Earnings: {formatAmount(payslip.totalEarnings)}
                </p>
                <p className="mt-1 text-lg text-slate-600">
                  Total Deductions: {formatAmount(payslip.totalDeductions)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2">
            <div className="border-r border-slate-300">
              <div className="grid grid-cols-[1fr_180px] border-b border-slate-300 bg-slate-50">
                <div className="border-r border-slate-300 px-5 py-4 text-xl font-black uppercase text-slate-900">Earnings</div>
                <div className="px-5 py-4 text-right text-xl font-black uppercase text-slate-900">Amount</div>
              </div>
              ...
              <div className="grid grid-cols-[1fr_180px] bg-slate-50">
                <div className="border-r border-slate-300 px-5 py-4 text-xl font-black text-slate-900">Gross Earnings</div>
            ...
            <div>
              <div className="grid grid-cols-[1fr_180px] border-b border-slate-300 bg-slate-50">
                <div className="border-r border-slate-300 px-5 py-4 text-xl font-black uppercase text-slate-900">Deductions</div>
                <div className="px-5 py-4 text-right text-xl font-black uppercase text-slate-900">Amount</div>
              </div>
              {Array.from({ length: maxRows }).map((_, index) => {
              ...
              <div className="grid grid-cols-[1fr_180px] bg-slate-50">
                <div className="border-r border-slate-300 px-5 py-4 text-xl font-black text-slate-900">Total Deductions</div>
          ...
          <div className="border-t border-slate-300">
            <div className="grid grid-cols-[1fr_260px] border-b border-slate-300 bg-slate-50">
              <div className="border-r border-slate-300 px-5 py-4 text-xl font-black uppercase text-slate-900">Net Pay</div>
              <div className="px-5 py-4 text-right text-xl font-black uppercase text-slate-900">Amount</div>
            </div>
            <div className="grid grid-cols-[1fr_260px] border-b border-slate-200">
              <div className="border-r border-slate-200 px-5 py-4 text-lg font-medium text-slate-800">Gross Earnings</div>
              <div className="px-5 py-4 text-right text-lg font-semibold text-slate-900">{formatAmount(payslip.totalEarnings)}</div>
            </div>
            <div className="grid grid-cols-[1fr_260px] border-b border-slate-200">
              <div className="border-r border-slate-200 px-5 py-4 text-lg font-medium text-slate-800">Total Deductions</div>
            ...
            <div className="grid grid-cols-[1fr_260px] bg-slate-50">
              <div className="border-r border-slate-300 px-5 py-5 text-3xl font-black text-slate-900">Total Net Payable</div>
          ...
          <div className="border-t border-slate-300 px-8 py-8 text-center">
            <p className="text-2xl font-semibold text-slate-800">
              Total Net Payable <span className="font-black">{formatAmount(payslip.netSalary)}</span>
            </p>
            <p className="mt-3 text-base text-slate-500">
              This document reflects the payroll calculation generated by NADAS ERP Suite.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-8 border-t border-slate-300 px-8 py-8">
            <div className="space-y-3">
              <p className="text-center text-sm font-black uppercase tracking-[0.2em] text-slate-600">Employee Signature</p>
