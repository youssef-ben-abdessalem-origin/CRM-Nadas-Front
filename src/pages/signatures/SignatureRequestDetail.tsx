...
export default function SignatureRequestDetail() {
  ...
  if (!task) {
    return (
      <CRMLayout title="Signature Detail">
        <div className="p-6">
  ...
  return (
    <CRMLayout title={task.title}>
      <div className="flex flex-col gap-6 p-6">
        <div className="text-sm text-slate-400">Home / Signatures / {task.title}</div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.5fr,1fr]">
          <Card className="glass-morphism">
            <CardContent className="space-y-8 p-8">
              ...
              <div>
                <p className="text-lg font-semibold text-slate-200">Summary</p>
                <div className="mt-4 space-y-2 text-lg text-slate-300">
                  <p><span className="font-semibold text-white">Creation Date:</span> {new Date(task.createdAt).toLocaleDateString()}</p>
                  <p><span className="font-semibold text-white">{task.signerName}</span> - Employee - {task.status === "signed" ? "Signed" : "Waiting Signature"}</p>
                </div>
              </div>
              ...
              >
                {task.status === "signed" ? "View Signed File" : "Sign"}
              </Button>

              <div className="border-t border-slate-800 pt-8">
                <h2 className="text-3xl font-bold text-slate-100">Communication history</h2>
                <p className="mt-3 text-slate-400">
                  This document is tracked inside the signing workflow. Once validated, the signed export is saved locally and the status updates automatically.
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="glass-morphism">
            <CardContent className="space-y-4 p-8">
              <h2 className="text-3xl font-semibold text-slate-100">Your Information</h2>
